...
Требования кейса, по которым продукт можно проверить, — в
[task-brief.md](task-brief.md). Формулы и допущения — в
...
`buildCheckpoint` — чистая функция от событий, таксономии и допущений о стоимости. Она не
знает про файловую систему и работает одинаково в обеих средах:

| Данные | Где считается | Почему так |
| --- | --- | --- |
| Демо-датасет | Node.js route handler | лог лежит в репозитории, расчёт кэшируется в памяти процесса |
| Загруженный лог | браузер пользователя | лог не должен покидать машину владельца |

Единственное исключение — пятый слой: ключ провайдера не может попасть в браузер, поэтому
клиент обращается к `/api/classify`. Туда уходит извлечённый вопрос, а не лог, и число
намерений за один запрос ограничено.

## Runtime

- одно приложение Next.js 16;
- route handlers работают в Node.js runtime;
- база данных, хранилище и отдельные workers отсутствуют;
...
src/lib/analytics/checkpoint.ts         сборка результата, изоморфная
src/lib/analytics/dataset.ts            разбор загруженного лога
src/lib/analytics/remote-llm.ts         слой модели через свой сервер
src/lib/analytics/problems.ts           сетка «что чинить»
...
src/app/api/checkpoint/                 демо-датасет
src/app/api/classify/                   пятый слой для браузера
src/app/api/sample-log/                 пример формата для загрузки
src/app/checkpoint/                     dashboard
src/components/DatasetLoader.tsx        drag-and-drop загрузка
```

## Что чинить

`src/lib/analytics/problems.ts` строит сетку «сценарий × режим отказа». Порог тревоги
считается от базовой линии самого лога: доля выше базовой в 1,3 раза — внимание, в 1,75 —
действие. Ниже шумового порога или меньше двадцати наблюдений — только внимание, никогда
действие. Каждая ячейка несёт числитель и знаменатель.
...
- у загруженного лога нет gold-разметки, поэтому блок качества классификации по нему
  ничего не утверждает;
- допущения о постоянных затратах берутся демонстрационные и пропорционально
  масштабируются по календарному охвату лога — под чужую компанию их надо задавать явно.
